...
class FermentationControl(object):
  def __init__(self, fermenterid):
    ...
    self.chamber_target_temp = self.fermenter()["target_temp"]
  ...
  def update_settings(self):
    ...
    if beer_temp_diff < 0: #linearly go to cool parameters in 3 hours
      self.Kp = constrain(self.Kp+(KpCool-KpHeat)/(360*3), KpCool, KpHeat)
      self.Kd = constrain(self.Kd+(KdCool-KdHeat)/(360*3), KdHeat, KdCool)
  ...
  def update_state(self):
    if self.state == "IDLE":
      lastIdleTime=datetime.datetime.utcnow()
  ...
  def detect_peaks(self):
    ...
    if self.do_neg_peak_detect and state!="COOLING":
